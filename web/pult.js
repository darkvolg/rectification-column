...
function jrnRow(){
  ...
  return {
    ...
    voda:  f(V.voda, 2),
    volt:  f(V.volt, 0),
    // Давление своё на каждую строку, а не одно на весь погон. Точка
    // кипения спирта плывёт на 0.037 °C на миллиметр, а за десять часов
    // погоды набегает и десять миллиметров — это 0.37 °C, то есть
    // больше, чем разница, по которой ловят переход на хвосты.
    press: f(V.press, 1),
    ml:    '',
    rate:  f(V.rate, 0),
    note:  ''
...
const JMEAS  = ['kub', 'otbor', 'carga', 'voda', 'volt', 'press', 'rate'];  // меряет прибор
...
const SD_COL = {kub: 1, otbor: 3, carga: 2, voda: 13, volt: 10, press: 15, rate: 16};
